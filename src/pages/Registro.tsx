      ...
      <div className="container py-20 max-w-md text-center">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
          ...
          <p className="text-muted-foreground mt-3">
            Te enviamos un email a <span className="font-semibold text-foreground">{email}</span> para confirmar tu cuenta.
          </p>
          <Button onClick={() => navigate("/")} className="mt-6 brand-gradient-bg text-primary-foreground rounded-full px-8">
            Ir al inicio
          </Button>
        </motion.div>
      </div>
        ...
        <div className="bg-card border rounded-2xl p-6 card-elevated">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} required placeholder="Ej: María González" />
            </div>
            <div className="space-y-2">
            ...
            >
              {loading ? <Loader2 size={18} className="animate-spin" /> : <>Crear cuenta <ArrowRight size={16} /></>}
            </Button>
          </form>
